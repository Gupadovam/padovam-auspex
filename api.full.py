...
experiment_root = "/home/lacerda/Desktop/ENSAIOS/06-11-23/"
experiment_ref = "api_full_ref.m2k"
...
img = np.log10(envelope(np.sum(data_ref.ascan_data, axis=2), axis=0) + 0.5)
...
plt.imshow(img, extent=[ang_span_original[0], ang_span_original[-1], t_span_original[-1], t_span_original[0]], cmap='magma', aspect='auto',
...
for i in range(22, 9, -1):
    print(f"Progresso:{i/31 * 100:.1f} %")
    j = i + 1
    experiment_name = f"api_full_{j:02d}.m2k"
    data = file_m2k.read(experiment_root + experiment_name, type_insp='contact', water_path=0, freq_transd=5,
                                      bw_transd=0.5, tp_transd='gaussian', sel_shots=0)[0]
    ...
    # Corta o A-scan para limites definidos:
    data_ref.ascan_data = data_ref.ascan_data[t0:tend, ang0:ang_end, :, :]
    data.ascan_data = data.ascan_data[t0:tend, ang0:ang_end, :, :]
    ...
    sscan_exp = envelope(np.sum(data.ascan_data - data_ref.ascan_data, axis=2), axis=0)
    sscan_exp_log = np.log10(sscan_exp + log_cte)

    # APlicação da API:
    corners = [(648, 25 + (22 - i) * 8), (803, 70 + (22 - i) * 8)]
    api_vec[22 - i], maxAngIdx, img_masked, maxPixel[22 - i] = api_func(sscan_exp, corners, thresh=.5)
    maxAng[22 - i] = ang_span[maxAngIdx]
    ...
    if (j % 2 == 0) or (j == 11):
        ...
        plt.imshow(sscan_exp_log, extent=[-49, 49, t_span[-1], t_span[0]], cmap='magma', aspect='auto',
                   interpolation="None", vmin=vmin_sscan, vmax=vmax_sscan)
        plot_echoes(62.83, 0, n_echoes=1, color='blue', xbeg=-40, xend=40)
